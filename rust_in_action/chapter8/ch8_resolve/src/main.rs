...
fn main() {
    ...
    #[cfg(feature = "foo")]
    {
        println!("Args:dns-server:{}", app.value_of("dns-server").unwrap());
        println!("Args:domain-name:{}", app.value_of("domain-name").unwrap());
    }
    ...
    let localhost = UdpSocket::bind("0.0.0.0:0").expect("cannnot bind to local socket");
    ...
    let dns_message = Message::from_vec(&response_as_bytes).expect("unabble  to parse response");
    for answer in dns_message.answers() {
        if answer.record_type() == RecordType::A {
            let resource = answer.rdata();
            let ip = resource.to_ip_addr().expect("invalid IP adddress received");
