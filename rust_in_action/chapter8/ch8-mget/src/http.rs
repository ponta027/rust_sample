...
pub fn get(
    ...
    let mut route = Routes::new(BTreeMap::new());
    let default_gateway = Ipv4Address::new(192, 168, 42, 100);
    route.add_default_ipv4_route(default_gateway).unwrap();
    ...
    let tcp_handler = sockets.add(tcp_socket);
    ...
    'http: loop {
        ...
        {
            let mut socket = sockets.get::<TcpSocket>(tcp_handler);
            state = match state {
