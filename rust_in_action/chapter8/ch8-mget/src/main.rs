...
fn main() {
    let app = App::new("mget")
        .about("GET a Webpge , manually")
        .arg(Arg::with_name("url").required(true))
        .arg(Arg::with_name("tap-device").required(true))
        .arg(Arg::with_name("dns-server").default_value("1.1.1.1"))
        .get_matches();
    println!("url={:}", app.value_of("url").unwrap());
    println!("tap-device={:}", app.value_of("tap-device").unwrap());
    println!("dns-server={:}", app.value_of("dns-server").unwrap());
    ...
    let tap = TapInterface::new(&tap_text)
        .expect(" error :uneable to use <tap-device> as a network interface");
    let domain_name = url.host_str().expect("domain name required");

    let _dns_server: std::net::Ipv4Addr = dns_server_text
        .parse()
        .expect("error:unable to parse< dns-sever> as ans IPv4 address");
    //
